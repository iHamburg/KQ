...
//    return [[CustomBarButtonItem alloc] initWithImage:image title:title target:target action:action];
    ...
    return [[UIBarButtonItem alloc] initWithCustomView:[UIButton buttonWithImageName:@"icon_setting.png" target:self action:action]];
}

+ (UIBarButtonItem *) barButtonItemWithImageName:(NSString *)imageName target:(id)target action:(SEL)action{
    return  [[UIBarButtonItem alloc] initWithCustomView:[UIButton buttonWithImageName:imageName target:target action:action]];
}
@end
