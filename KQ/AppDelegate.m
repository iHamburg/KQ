...
- (NSManagedObjectModel *)managedObjectModel
{
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"Makers" withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"Makers.sqlite"];
...
- (void)initVariables
{
    ...
    _h = isIOS7?_r.size.height:_r.size.height-20;
    
    
    //    NSLog(@"bundleid # %@",[[NSBundle mainBundle] bundleIdentifier]);
    
    //        NSLog(@"isios7 # %d,yoffset # %f, simulator # %d",isIOS7,_yOffset,isSimulatorVersion());
    
    //    NSLog(@"rect # %@,_h # %f",NSStringFromCGRect(_r),_h);
...
- (void)fonts{
    ...
    for(NSString *familyName in array)
    {
        ...
        NSArray *os = [UIFont fontNamesForFamilyName:familyName];

//        if ([familyName isEqualToString:kFontHeitiSC]) {
//              NSArray *os = [UIFont fontNamesForFamilyName:familyName];
            NSLog(@"font name # %@",os);
//        }
